...
Prompt = '''Separate only those details that the document contains such as name, dob, gender, document Id, address, license class, date of issue, date of expiry with exclamatory mark(!) and Return the text from the image as in the below examples:

Examples:
Whatever the information that the document had return in the below format:
    Name : John Doe
    DOB : 01/01/1990
...
Aadhar_prompt = '''Separate the name, dob, gender, Aadhar Number, address(if found) with exclamatory mark(!) and return the text in capital letters from the image in this format only:
Name : Name from the document
DOB :  DOB from the document
GENDER: Gender from the document
AADHAR Number : Passport number from the document
...
def process_document(details_text, image):
    ...
    if uploaded_file is not None:
        ...
        if submit:
            ...
            st.download_button(
                label=f"Download Details as JSON",
                data=json.dumps(edited_details_dict, indent=4),
                file_name=f"details.json",
            ...
            st.download_button(
                label=f"Download Details as CSV",
                data=csv_data,
